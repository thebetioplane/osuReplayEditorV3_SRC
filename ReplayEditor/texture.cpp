...
#define glRotate(r, x, y, z) glRotatef(static_cast<GLfloat>(r), static_cast<GLfloat>(x), static_cast<GLfloat>(y), static_cast<GLfloat>(z))

texture_t::texture_t(int w, int h, void *data, GLint tex_filter)
	: width(w), height(h)
{
	glGenTextures(1, &id);
...
#define INIT_TEX(var_name, w, h) do { var_name = texture_t::from_file(L"assets/" #var_name L".raw", w, h, GL_LINEAR); if (var_name == nullptr) return false; } while(0)
