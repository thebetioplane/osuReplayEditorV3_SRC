...
static void setup_graphics(HWND hWnd)
{
	...
	if (iFormat == 0)
		fatal("ChoosePixelFormat failed");
	if (!SetPixelFormat(graphics_handle.hdc, iFormat, &pfd))
		fatal("SetPixelFormat failed");
	graphics_handle.hrc = wglCreateContext(graphics_handle.hdc);
	if (graphics_handle.hrc == nullptr)
		fatal("wglCreateContext failed");
	if (!wglMakeCurrent(graphics_handle.hdc, graphics_handle.hrc))
		fatal("wglMakeCurrent failed");
}
...
DLLFUN(INT) Init(HWND hWnd, const wchar_t *osu_db_path, const wchar_t *song_path)
{
	...
	if (!osudb::init())
		return OSUDB_FAILURE;
	if (!replayengine::init(L""))
		return REPLAYENGINE_FAILURE;
	if (!beatmapengine::init(L""))
		return BEATMAPENGINE_FAILURE;
...
DLLFUN(void) AnalyzeAccuracy(BOOL do_trace, int *num_300, int *num_100, int *num_50, int *num_miss, float *accuracy, float *avg_early, float *avg_late, float *unstable_rate)
{
...
DLLFUN(INT) NextMiss()
{
	return accuracy_analyzer::next_hitobject([](const hitobject_t& obj) {
		return obj.is_miss;
	});
}
...
DLLFUN(INT) Next50()
{
	...
	return accuracy_analyzer::next_hitobject([=](const hitobject_t& obj) {
		return obj.get_points(w50, w100, w300) == 50;
	});
}
...
DLLFUN(INT) Next100()
{
	...
	const int w300 = beatmapengine::hitwindow300();
	return accuracy_analyzer::next_hitobject([=](const hitobject_t& obj) {
		return obj.get_points(w50, w100, w300) == 100;
	});
}
...
DLLFUN(INT) Next300()
{
	...
	const int w300 = beatmapengine::hitwindow300();
	return accuracy_analyzer::next_hitobject([=](const hitobject_t& obj) {
		return obj.get_points(w50, w100, w300) == 300;
	});
}
...
{
	if (index < 0 || index >= beatmapengine::hitobjects.size())
		return FALSE;
