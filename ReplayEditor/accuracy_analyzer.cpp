...
void accuracy_analyzer::analyze(Stats* stats, bool do_trace)
{
	std::ofstream log;
	if (do_trace)
		log.open("accuracy_analyzer.log");
	if (!log.good())
		do_trace = false;
	...
	if (do_trace) {
		...
		log << "++++++++++++++++++++++++++++++++" << std::endl;
		log << "SldTk means slider tick; bitmap is prev frame pressed, curr frame pressed, inside slider ball" << std::endl;
		log << "++++++++++++++++++++++++++++++++" << std::endl;
	}
	...
	while (replayframe_index < replayengine::frames.size() && hitobject_index < beatmapengine::hitobjects.size()) {
		...
		const bool is_press = IS_PRESSED(prev_frame.pressed_mouse1(), prev_frame.pressed_mouse2(), curr_frame.pressed_mouse1(), curr_frame.pressed_mouse2());
		...
		if (curr_obj.hitobject_type == HitObjectType::SliderTick) {
			...
			if (beatmapengine::hitobjects_inverted)
				hitobj_pos.y = 384.0 - hitobj_pos.y;
			const bool inside_slider_ball = glm::distance(hitobj_pos, curr_frame.p) <= ball_radius;
			if (do_trace)
				log << "SldTk: Object #" << hitobject_index << "; Frame #" << replayframe_index << "; " << prev_frame_pressed << curr_frame_pressed << inside_slider_ball << std::endl;
			...
			if (curr_obj.hitobject_type == HitObjectType::Spinner) {
				if (do_trace)
					log << "Press: Object #" << hitobject_index << "; Frame #" << replayframe_index << "; Spinner skipped" << std::endl;
			...
			if (curr_frame.ms > curr_obj.start + window50) {
				if (do_trace)
					log << "Press: Object #" << hitobject_index << "; Frame #" << replayframe_index << "; Miss - Past hit window" << std::endl;
			...
			if (glm::distance(hitobj_pos, curr_frame.p) > radius) {
				if (do_trace)
					log << "Press: Object #" << hitobject_index << "; Frame #" << replayframe_index << "; Ignore - missed hitcircle" << std::endl;
			...
			if (std::abs(error) <= window50) {
				if (do_trace)
					log << "Press: Object #" << hitobject_index << "; Frame #" << replayframe_index << "; Hit (" << error << " ms)" << std::endl;
				curr_obj.is_miss = false;
	...
	for (const auto& obj : beatmapengine::hitobjects) {
		if (!obj.is_miss)
			variance.add(std::pow(obj.hit_error - avg, 2.f));
	}
	stats->accuracy = 100.f * RATIO(stats->num_50 + 2 * stats->num_100 + 6 * stats->num_300, 6 * beatmapengine::hitobjects.size());
...
{
	auto iter = std::lower_bound(beatmapengine::hitobjects.begin(), beatmapengine::hitobjects.end(), audioengine::get_time(),
		[](const hitobject_t& lhs, SongTime_t rhs)
	{
