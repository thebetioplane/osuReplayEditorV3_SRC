...
static void draw_frames(const SongTime_t ms)
{
	...
	if (ui::trail_mode == ui::TrailMode::Off)
		return;
	...
	if (ui::trail_mode == ui::TrailMode::Raw) {
		...
		for (auto curr = start + 1; curr != end; ++curr, ++prev) {
			const bool is_press = IS_PRESSED(prev->pressed_mouse1(), prev->pressed_mouse2(), curr->pressed_mouse1(), curr->pressed_mouse2());
			const bool is_release = IS_RELEASE(prev->pressed_mouse1(), prev->pressed_mouse2(), curr->pressed_mouse1(), curr->pressed_mouse2());
			draw_marks(curr);
			if (is_press)
				glColorYellow();
			else if (is_release)
	...
	}
	{
		...
		for (auto curr = start; curr != end; ++curr) {
			...
			const bool is_release = IS_RELEASE(prev_mouse1, prev_mouse2, curr->pressed_mouse1(), curr->pressed_mouse2());
...
{
	if (!is_mark_index_in_range(mark_in))
		return false;
	if (!is_mark_index_in_range(mark_out))
		return false;
	if (!is_mark_index_in_range(mark_mid))
		return false;
