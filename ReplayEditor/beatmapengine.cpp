...
static bool timing_point_at(SongTime_t ms, timing_point_t &tp)
{
	...
	auto item = std::upper_bound(timing_points.begin(), timing_points.end(), ms, [](const SongTime_t lhs, const timing_point_t &rhs) {
		return lhs < rhs.ms;
	});
	// when the start points are equal use found timing point
	// otherwise go to the previous one
	if (item == timing_points.end() || (item != timing_points.begin() && ms != item->ms))
		--item;
	tp = *item;
	return true;
}
...
static void update_stacking(int start_index, int end_index)
{
	...
	for (int i = start_index; i <= end_index; ++i)
		beatmapengine::hitobjects[i].stack_count = 0;
	...
	for (int i = end_index; i >= start_index; --i) {
		...
		for (int n = stack_base_index + 1; n < hitobject_count; ++n) {
			...
			if (!base_obj.is_circle_or_slider())
				break;
			auto& obj = beatmapengine::hitobjects[n];
			if (!base_obj.is_circle_or_slider())
				continue;
			if (obj.start - base_obj.end > stack_threshold)
				break;
		...
		if (stack_base_index > extended_end_index) {
			...
			if (extended_end_index == hitobject_count - 1)
				break;
		}
	}
	...
	for (int i = extended_end_index; i > start_index; --i) {
		...
		if (obj_i->stack_count != 0 || !obj_i->is_circle_or_slider())
			continue;
		if (obj_i->slider == nullptr) {
			while (--n >= 0) {
				hitobject_t *obj_n = &beatmapengine::hitobjects[n];
				if (!obj_n->is_circle_or_slider())
					continue;
				if (obj_i->start - obj_n->end > stack_threshold)
					break;
			...
			while (--n >= start_index) {
				...
				if (!obj_n->is_circle_or_slider())
					continue;
				if (obj_i->start - obj_n->start > stack_threshold)
					break;
	...
	for (int i = start_index; i <= end_index; ++i) {
		...
		obj.pos += offset;
		if (obj.slider)
			obj.slider->translate_offset(offset);
	}
}
...
static void add_slider_ticks()
{
	...
	for (size_t i = 0; i < num_objects; ++i)
	{
		...
		{
			if (glm::abs(t - hitobjects[i].start - hitobjects[i].slider->duration()) <= slider_leniency)
				break;
			...
			hitobjects.emplace_back(pos, t, hitobjects[i].start + hitobjects[i].slider->duration(), HitObjectType::SliderTick);
...
bool beatmapengine::init(const std::wstring &fname)
{
	...
	while (std::getline(r, line)) {
		if (line.length() < 2)
			continue;
		if (line.find("Unicode") != std::string::npos)
			continue;
		if (std::find_if(line.begin(), line.end(), [](int ch) {
			return ch < ' ' || ch >= 128;
		}) != line.end()) {
			continue;
		}
		...
		case S_TimingPoints: {
			...
			sscanf_s(line.c_str(), "%f,%f,%d,%d,%d,%d,%d,%d", &ms_float, &beatlength, &meter, &sample_set, &sample_index, &volume, &uninherited, &effects);
			timing_point_t tp;
			...
			TRACE << "[" << (uninherited ? 'X' : ' ') << "] ms_float = " << ms_float << "; beatlength = " << beatlength << "; slider velocity = " << tp.slider_velocity << std::endl;
#endif
...
static void interval_tree_search(const hitobject_t *const root, const SongTime_t ms, std::vector<const hitobject_t*> &found)
{
	if (root == nullptr)
		return;
	if (root->max_in_subtree < ms)
		return;
