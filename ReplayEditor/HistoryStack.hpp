...
class HistoryStack
{
   ...
   public:
    ...
    {
        if (!ui::is_in_out_mark_consistent()) return false;
        const I64 count = ui::mark_out - ui::mark_in + 1;
        if (count <= 0) return false;
        HistoryFrame hframe{ui::mark_in, ui::mark_out, std::make_unique<replayengine::replayframe_t[]>(count)};
    ...
    bool restore_snapshot()
    {
        ...
        if (hframe.frames == nullptr) return false;
