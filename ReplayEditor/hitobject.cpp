...
{
	int kind;
	if (sscanf_s(def, "%f,%f,%d,%d", &pos.x, &pos.y, &start, &kind) != 4)
	...
	if (KIND_IS_CIRCLE) {
		...
		if (comma_ahead(def, 5))
			fatal("bad spinner def");
		if (sscanf_s(def, "%d", &end) != 1)
			fatal(".osr bad format");
...
hitobject_t::hitobject_t(const glm::vec2& my_pos, SongTime_t my_start, SongTime_t my_end, HitObjectType my_hit_object_type) noexcept
	: pos(my_pos), start(my_start), end(my_end), hitobject_type(my_hit_object_type), slider(nullptr), is_miss(true),
	hit_error(0), stack_count(0), left(nullptr), right(nullptr), max_in_subtree(MIN_SONG_TIME)
{
}
