...
static inline bool file_exists(const std::wstring &fname)
{
...
bool audioengine::load(const std::wstring &fname)
{
	...
	stream = BASS_StreamCreateFile(false, full_path.c_str(), 0, 0, BASS_STREAM_DECODE | BASS_UNICODE | BASS_STREAM_PRESCAN);
	if (stream == 0)
	{
