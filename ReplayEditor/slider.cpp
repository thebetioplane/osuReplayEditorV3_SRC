...
static float det3(const float a, const float b, const float c, const float d, const float e, const float f, const float g, const float h, const float i)
{
...
static float catmull_rom(const float v0, const float v1, const float v2, const float v3, float t)
{
	...
	return 0.5f * (2.f * v1 + (-v0 + v2) * t + (2.f * v0 - 5.f * v1 + 4.f * v2 - v3) * t_sq + (-v0 + 3.f * v1 - 3.f * v2 + v3) * t_cubed);
}
...
void slider_t::s_catmull(const std::vector<glm::vec2> &pts, SongTime_t start, SongTime_t end)
{
	...
	PosDist eval{
		eval_count,
		std::make_unique<glm::vec2[]>(eval_count),
		std::make_unique<float[]>(eval_count)
	};
	for (int i = 0; i < N - 1; ++i) {
...
void slider_t::s_circle(const std::vector<glm::vec2> &pts, SongTime_t start, SongTime_t end)
{
	...
	for (float f = from; glm::distance(prev, pts[2]) > 1.0f || num_segments < 2; f += step) {
		...
		prev = curr;
		++num_segments;
		if (distance_so_far >= pixel_length)
			break;
	}
}
...
slider_t *slider_t::from_def(const char *def, glm::vec2 pos, SongTime_t start, SongTime_t end)
{
	...
	for ( ;; ) {
		glm::vec2 pt;
		if (sscanf_s(def, "|%f:%f", &pt.x, &pt.y) != 2)
			break;
		pts.push_back(pt);
		++def;
	...
	if (pts.size() <= 1)
		fatal("slider not enough points defined");
	int repeat;
	float pixel_length;
	if (sscanf_s(def, ",%d,%f", &repeat, &pixel_length) != 2)
		fatal("bad slider def");
	...
	case 'P':
		if (pts.size() != 3
			|| is_collinear(pts[0], pts[1], pts[2])
			|| glm::distance(pts[0], pts[1]) < p_dist_thresh
			|| glm::distance(pts[1], pts[2]) < p_dist_thresh
	...
	std::stable_sort(obj->curve.begin(), obj->curve.end(), [](const pos_time_t& lhs, const pos_time_t& rhs)
	{
...
bool slider_t::position_at_time(SongTime_t ms, SongTime_t start, SongTime_t end, glm::vec2& out_pos) const
{
	if (ms >= start && ms <= end)
	{
		auto iter = std::upper_bound(curve.begin(), curve.end(), ms, [](const SongTime_t ms, const pos_time_t& pos_time) {
			return pos_time.second > ms;
		});
